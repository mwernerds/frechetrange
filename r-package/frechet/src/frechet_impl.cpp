...
/// Grid will cache the data outside of R to have valid container as some of the
/// implementations
// are not yet compatible. This is a huge memory-overhead and must be
// well-documented.
// otoh, it gives us highly efficient storage and allows to control memory
// layout, e.g. through
// boost::object_pool, where wanted.

template <size_t _DIM = 2> class GridDataset {
public:
  const size_t DIM = _DIM;
  typedef std::array<double, _DIM> point;
  ...
  std::function<double(const point &p)> getx = [](const point &p) {
    return p[0];
  };
  std::function<double(const point &p)> gety = [](const point &p) {
    return p[1];
  };
...
bool internal_createIndex_dv(size_t gds, double meshSize) {
  ...
  gData[gds].buildIndex(meshSize);
}

template <typename traj> NumericMatrix _trajectory2matrix(const traj &t) {
...
// [[Rcpp::export]]
List internal_gridRangeQuery(size_t gds, const NumericMatrix &m, double eps,
