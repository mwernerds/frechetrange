...
ds = internal_bb_create_index();
### Feed the data into C++
tmp = lapply(datalist, function(x) internal_bb_add_trajectory(ds,as.matrix(x)));

### Now, query
result = internal_bb_range_query(ds,as.matrix(datalist[[queryIndex]]),.02);
