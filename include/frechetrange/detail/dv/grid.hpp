    ...
    */
    void reserve(size_t numTrajectories) {
        for (size_t i = 0; i < NUM_MAPS; ++i) {
            if (!_optimized ||
                i == toMapIndex(_useLeftBorder, _useBottomBorder))
                _maps[i].reserve(numTrajectories);
        }
    }

    void insert(const Trajectory &trajectory) {
        if (trajectory.size() > 0) insertImpl(MBR(trajectory));
    }

    void insert(Trajectory &&trajectory) {
        if (trajectory.size() > 0) insertImpl(MBR(std::move(trajectory)));
    }
    ...
    */
    ...
    void insertInMap(MBR &&mbr) {
        ...
        if (!_optimized) {
            ...
            // (b + 1)^2 = b^2 + 2*(b+1) - 1
            _expectedQueryCost[toMapIndex(left, bottom)] +=
                2 * bucket.size() - 1;
        } else {
    ...
    void query(const Trajectory &query, double threshold,
               Output &output) const {
        MBR queryMBR(query);

        // check which horizontal neighbor cells need to be visited
        double cellCoordX =
            toCellCoord(queryMBR.template getBorder<true, left>());
        bool visitLeft =
            queryMBR.template getBorder<true, left>() - threshold < cellCoordX;
        bool visitRight =
            queryMBR.template getBorder<true, left>() + threshold >=
        ...
        bool visitBottom =
            queryMBR.template getBorder<false, bottom>() - threshold <
            cellCoordY;
        bool visitTop =
            queryMBR.template getBorder<false, bottom>() + threshold >=
        ...
        if (visitLeft) {
            ...
            --cellNr.y;
            checkCell<left, bottom, Output>(cellNr, queryMBR, threshold,
                                            CROSSES_END, crossedHorizontals, fd,
                                            output);
    ...
    void checkCell(const CellNr &cellNr, const MBR &queryMBR, double threshold,
        ...
        if (cell.xSorted) {
            ...
            this->template traverseBucket<false, bottom, Output>(
                cell.bucket, queryMBR, threshold, crossedHorizontals, fd,
                output);
        }
    }

    template <bool xDim, bool firstBorder, typename Output>
    void traverseBucket(const typename Cell::Bucket &bucket,
                        const MBR &queryMBR, double threshold,
                        Crossings crossedBorders, frechet_decider &fd,
                        Output &output) const {
        if (bucket.size() < MIN_SORT_SIZE || !_optimized) {
            ...
            for (const MBR &trajectory : bucket) {
                checkTrajectory<false, false, false>(queryMBR, threshold,
                                                     trajectory, fd, output);
            }
            ...
            if (crossedBorders == CROSSES_END) {
                ...
                double activeRangeBegin =
                    queryMBR.template getBorder<xDim, firstBorder>() -
                    threshold;
                for (size_t i = bucket.size() - 1;
                    ...
                    checkTrajectory<true, xDim, firstBorder>(
                        queryMBR, threshold, bucket[i], fd, output);
                    ...
                    if (useBinarySearch(bucket.size(), threshold)) {
                        ...
                        searchBegin = std::lower_bound(
                            ...
                            [](const MBR &m, double d) {
                                return m.template getBorder<xDim,
                                                            firstBorder>() < d;
                            });
                    } else {
                        // linear search for the beginning of the active range
                        while (searchBegin != searchEnd &&
                               searchBegin->template getBorder<xDim,
                                                               firstBorder>() <
                                   activeRangeBegin) {
                            ++searchBegin;
                        }
                ...
                while (searchBegin != searchEnd &&
                       searchBegin->template getBorder<xDim, firstBorder>() <=
                           activeRangeEnd) {
                    checkTrajectory<true, xDim, firstBorder>(
                        queryMBR, threshold, *searchBegin, fd, output);
                    ++searchBegin;
                }
            }
        }
    ...
    bool mbrsWithinRange(const MBR &queryMBR, double threshold,
        ...
        if (prechecked) {
            ...
            // ensure that all bounding box borders are within range
            return bordersWithinRange<true, true>(queryMBR, threshold,
                                                  trajMBR) &&
                   bordersWithinRange<true, false>(queryMBR, threshold,
                                                   trajMBR) &&
                   bordersWithinRange<false, true>(queryMBR, threshold,
                                                   trajMBR) &&
                   bordersWithinRange<false, false>(queryMBR, threshold,
                                                    trajMBR);
        }
    }
