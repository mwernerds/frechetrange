...
template <typename Trajectory, typename xgetterfunctional, typename ygetterfunctional>
class SpatialHash {
	struct Point;
...
public:
  ...
  void insert(const Trajectory &trajectory) {
    if (trajectory.size() > 0) {
      ...
      _extTrajectories.emplace_back(trajectory, _extTrajectories.size(), _getX, _getY);
	}
  }
  ...
  */
  ...
  void rangeQuery(const Trajectory &query, double distanceThreshold,
		...
		makeSourceSimplificationsForTrajectory(queryTrajectory, queryTrajectory, diagonal, agarwalProg, numSimplifications);
		for (int i = 1; i < numSimplifications; i++) {
			makeSourceSimplificationsForTrajectory(*(queryTrajectory.simplifications[i]), queryTrajectory, diagonal, agarwalProg, i-1);
		}

		CDFQShortcuts cdfqs;
		collectDiHashPoints(queryTrajectory, distanceThreshold, [&](const ExtTrajectory &t) {
			pruneWithSimplifications(cdfqs, queryTrajectory, distanceThreshold, t, [&](const ExtTrajectory &t) {
				pruneWithEqualTime(queryTrajectory, distanceThreshold, t, [&](const ExtTrajectory &t) {
					pruneWithDecisionFrechet(cdfqs, queryTrajectory, distanceThreshold, t, output);
				}, output);
			}, output);
		});
	}
...
private:
  ...
  ygetterfunctional _getY;
	...
	struct Portal {
		...
		bool operator<(const Portal &p) const { return destination < p.destination; };
	};
	...
	struct ExtTrajectory {
		...
		std::vector<size_t> sourceIndex;// mapping back to the source trajectory if this is a simplification
		...
		void addSimplification(TrajectorySimplification&& simp) {
			simplifications.emplace_back(new TrajectorySimplification(std::move(simp)));
		}
	...
	struct TrajectorySimplification : public ExtTrajectory {
		...
		TrajectorySimplification(double eps) : simplificationEpsilon(eps), portals() {}
	};

	static double equalTimeDistance(const ExtTrajectory &p, const ExtTrajectory &q) {
	...
	static double equalTimeDistance(
		const Vertices &pverts, const Vertices &qverts,
		const std::vector<double> &ptotals, const std::vector<double> &qtotals,
		const std::vector<double> &pdistances, const std::vector<double> &qdistances,
		size_t psize, size_t qsize,
		size_t pstart, size_t qstart) {
		...
		while (!(p_ptr == psize - 1 && q_ptr == qsize - 1)) {
			...
			if (p_ptr == psize - 1) nextDistP = std::numeric_limits<double>::max();
			if (q_ptr == qsize - 1) nextDistQ = std::numeric_limits<double>::max();
	...
	static bool computeInterval(const Point &a, const Point &b1, const Point &b2, double eps, Range &r) {
	...
	class DiHash {
		...
		const int _slotsPerDimension; //number of equally spaced subdivisions in each dimension
		std::vector<std::vector<Vertex>> _elements;
		double _tolerance; // tolerance to prevent numerical representation errors
		double _limits[2][2]; // two rows (x,y) and two columns(min,max), keeps the information on limits in each dimension, for 3D, just add one row.
	
	public:
		DiHash(int numC, double tol) : _slotsPerDimension(numC), _elements(numC*numC), _tolerance(tol) {}
		...
		void addPoint(const Point& p, const ExtTrajectory &t, bool isStart) {
			...
			_elements[xSlot*_slotsPerDimension + ySlot].push_back(Vertex{p.x, p.y, t, isStart});

		}

		// Return neighbors at range strictly less than distance for a given point, does not return the query point if present
		void neighborsWithCallback(const Point &start, const Point& end, double eps, std::vector<ExtTrajectory> &trajectories, const std::function< void(const ExtTrajectory&) >& emit) {
	...
	private:
		...
		std::pair<int, int> slotsTouched(double min, double max, char type) {
			...
			return std::make_pair(findSlot(min, type, true), findSlot(max, type, true));
		}
		...
		int findSlot(double val, char type, bool allowOverflow) {
			...
			if (fabs(max - val) < _tolerance) return _slotsPerDimension - 1;
			else if (fabs(min - val) < _tolerance) return 0;
			else {
				double pas = (fabs(max - min) / _slotsPerDimension);
				int retorn = (int)((val - min) / pas);
				
				if (retorn >= _slotsPerDimension) return _slotsPerDimension - 1;
				else if(retorn < 0) return 0;
				else return retorn;
			}
		}
	};
	...
	void makeSimplificationsForTrajectory(ExtTrajectory &t, double diagonal, AgarwalSimplification &agarwal, int size) {
	...
	// TODO: improve the epsilon to apply to more types of trajectories,
	// TODO: or track the simplification coarseness and adapt epsilon based on it.
	void makeSourceSimplificationsForTrajectory(ExtTrajectory &t, ExtTrajectory &source, double diagonal, ProgressiveAgarwal &agarwalProg, int size) {
		for (int i = 0; i < size; i++) {
	...
	// to candidates.
	void collectDiHashPoints(const ExtTrajectory &queryTrajectory, double queryDelta, const std::function< void(const ExtTrajectory&) >& emit) {
		const auto &start = queryTrajectory.front();
		const auto &end = queryTrajectory.back();
		_diHash.neighborsWithCallback(start, end, queryDelta, _extTrajectories, emit);
	}
	...
	void makeSimplificationsForTrajectory(ExtTrajectory &t, AgarwalSimplification &agarwal) {
		makeSimplificationsForTrajectory(t, t.boundingBox.getDiagonal(), agarwal, numSimplifications);
	}
...
#ifdef USE_MULTITHREAD
		const size_t numWorkers = std::thread::hardware_concurrency(); // worker threads == number of logical cores
		std::vector<std::thread> simplificationThreads;
		std::vector<BoundingBox> bboxes(numWorkers);

		_startedSimplifying = 0;
		for (size_t i = 0; i < numWorkers; i++) {
			simplificationThreads.emplace_back(&SpatialHash::simplificationWorker, this, &(bboxes[i]));
		}
		for (size_t i = 0; i < numWorkers; i++) {
	...
	*/
  template <typename OutputFunctional>
	void returnTrajectory(const ExtTrajectory &et, OutputFunctional &output) const {
  ...
  template <typename OutputFunctional>
	void pruneWithSimplifications(CDFQShortcuts &cdfqs, const ExtTrajectory &queryTrajectory, double queryDelta, const ExtTrajectory &t, 
		const std::function< void(const ExtTrajectory&) >& maybe, OutputFunctional &output) {
		bool broke = false;
		for (int i = 0; i < numSimplifications; i++) {

			double decisionEpsilonLower = queryDelta
				- queryTrajectory.simplifications[i]->simplificationEpsilon
				- t.simplifications[i]->simplificationEpsilon;

			double decisionEpsilonUpper = queryDelta
				+ queryTrajectory.simplifications[i]->simplificationEpsilon
				+ t.simplifications[i]->simplificationEpsilon;
			...
			if (decisionEpsilonLower > 0) {
				bool r = cdfqs.calculate(*queryTrajectory.simplifications[i], *t.simplifications[i], decisionEpsilonLower, queryDelta);
				if (r) {
					returnTrajectory(t, output);
					broke = true;
					break;
			...
			if (decisionEpsilonUpper > 0) {
				bool r = cdfqs.calculate(*queryTrajectory.simplifications[i], *t.simplifications[i], decisionEpsilonUpper, queryDelta);
				if (!r) {
					broke = true;
					break;
				}
  ...
  template <typename OutputFunctional>
	void pruneWithEqualTime(const ExtTrajectory &queryTrajectory, double queryDelta, const ExtTrajectory &t,
		const std::function< void(const ExtTrajectory&) >& maybe, OutputFunctional &output) {
  ...
  template <typename OutputFunctional>
	void pruneWithDecisionFrechet(CDFQShortcuts &cdfqs, const ExtTrajectory &queryTrajectory, double queryDelta, const ExtTrajectory &t,
		OutputFunctional &output) {
...
// -----------------------------------------
	...
	class CDFQShortcuts {
		...
		double computeSegmentFrechet(
			const Portal &p,
			int q,
			const Vertices &p_array, const Vertices &q_array
		) {
	...
	public:
		int numRows = 0;
		bool calculate(
			const Vertices &P, const Vertices &Q,
			...
			if (startDist > queryDelta*queryDelta || endDist > queryDelta*queryDelta) return false;
					...
					do {
						...
						if (RFree) {
							if (left_most_top <= 1) {
								...
								if (Rf.isComplete() && queueSize[second] > 0 && queue[second][queueSize[second] - 1].end_row_index == row - 1) {
								...
								if (!outsideQueue && row >= queue[first][qIndex].start_row_index && row <= queue[first][qIndex].end_row_index) {
									if (!(row == queue[first][qIndex].start_row_index && queue[first][qIndex].lowest_right > Rf.end)) {
										...
										if (Rf.isComplete() && newLR == 0.0 && queueSize[second] > 0 && queue[second][queueSize[second] - 1].end_row_index == row - 1) {
											// complete reachable right means increase previous queue entry to span this cell
											queue[second][queueSize[second] - 1].end_row_index = row;
										}
										else {
	...
	// accepts lambda function returning whether the given search index
	// satisfies the search criterion.
	static int doubleNsearch(const std::function< bool(int) >& f, int start, int end, int doubleNSearchBase, double doubleNSearchExponentStep) {
		int k = start;
		int prevk = start;
		int iteration = 0;
		while (true) {
	...
	static int binaryIntSearch(const std::function< bool(int) >& f, int upperbound, int lowerbound) {
	...
	private:
		...
		int findLastFrechetMatch(
			const Vertices &P, Vertices &simp,
